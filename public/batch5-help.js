(() => {
  ...
  const params=new URLSearchParams(location.search),moduleId=params.get('module'),config=CONFIG[moduleId];if(!config)return;
  ...
  function solution(item,heading='Correct answer and full explanation'){
    if(!item)return'';const source=item.solution_steps?.length?item.solution_steps:item.alternate_solution_steps||[];const steps=source.map((s,i)=>`<li><span class="solution-step-number">${i+1}</span><div><div class="math-line">${esc(s.equation)}</div><p>${esc(s.explanation)}</p></div></li>`).join('');
  ...
  function strengthen(){
    const f=document.querySelector('#lessonFeedback');if(!f||isMastery()||f.querySelector('[data-batch5-reveal="true"]'))return;const text=f.textContent||'',item=itemMap.get(currentId());if(!item)return;
    ...
    paused(()=>{
      f.insertAdjacentHTML('beforeend',solution(item,wrong?'Check your work: answer and full solution':'Full explanation and answer'));
      if(wrong){const answer=document.querySelector('#lessonAnswer'),check=document.querySelector('#submitLessonAnswer'),next=document.querySelector('#nextLessonStep');if(answer)answer.disabled=true;if(check)check.disabled=true;if(next){next.textContent='Review Solution & Continue →';next.style.display='inline-block';}}
    });
  }
  async function start(){try{const r=await fetch(config.path);if(r.ok){const m=await r.json();itemMap=new Map((m.items||[]).map(i=>[i.id,i]));}}catch(e){console.warn('Batch help bank unavailable',e);}const f=document.querySelector('#lessonFeedback');if(!f)return;observer=new MutationObserver(strengthen);observer.observe(f,{childList:true,subtree:true});strengthen();}
