(() => {
  ...
  if (moduleId !== "alg1-a9a-exponential-domain-range") return;
  ...
  function richSolution(config, heading = "Here is the complete solution") {
    return `
  ...
  function upgradeA9aHelp() {
    ...
    if (/Correct\./i.test(text)) {
      feedback.innerHTML = richSolution(config, "Correct — here is why");
      return true;
    }
  ...
  function addSolutionButton() {
    ...
    if (!config) return;
    const text = feedback.textContent || "";
    if (feedback.querySelector(".tolux-show-solution") || feedback.querySelector(".tolux-final-answer")) return;
    if (!/Not quite yet|Try the problem again|Hint\s+[12]|Another way/i.test(text)) return;
    ...
    button.addEventListener("click", () => {
      feedback.innerHTML = richSolution(config);
    });
