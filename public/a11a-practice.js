...
function buildPracticeItems(module, difficulty, count) {
  ...
  if (bank.length < count) throw new Error("A.11A does not yet contain enough verified practice items.");
  ...
  return ordered.slice(0, count).map((item, index) => ({
    ...
    hint: item.hint_steps?.[0] || item.tutor_behavior || "Find a perfect-square factor first.",
    alternate_explanation: item.alternate_solution_steps?.length
      ? "Use the alternate worked steps below."
      : item.explanation_guidance || item.tutor_behavior || "Rewrite the radicand using a perfect-square factor, then simplify.",
    solution_steps: item.solution_steps?.length
      ? item.solution_steps
      : item.alternate_solution_steps?.length
        ? item.alternate_solution_steps
        : [
            { equation: item.prompt, explanation: item.tutor_behavior || "Identify a useful perfect-square factor." },
            { equation: item.answer_key, explanation: "Check that no perfect-square factor greater than 1 remains inside the radical." }
          ]
  }));
}
...
function fullHelpMarkup(item) {
  ...
  return `
    <div class="lesson-state lesson-state-success">
      ...
      <p>Look for a perfect-square factor, rewrite the radicand as a product, take the square root of the perfect-square factor, and then combine like radicals if needed.</p>
    </div>
...
async function ensureAccess(item) {
  ...
  try {
    ...
    console.error("A.11A practice access failed:", error);
...
async function heartbeat() {
  ...
  try {
    ...
    console.error("A.11A heartbeat failed:", error);
  }
}
...
function renderQuestion() {
  ...
  els.progressLabel.textContent = `A.11A • Question ${number} of ${practiceSession.count}`;
  ...
  els.answer.value = "";
  els.answer.placeholder = "Example: 6√2 or 6sqrt(2)";
...
async function checkAnswer() {
  ...
  if (record.attempt_count === 1) {
    ...
    setFeedback(`<div class="lesson-state lesson-state-warning"><strong>Not correct yet</strong><p>Look for the largest perfect-square factor, simplify each radical completely, and combine only like radicals.</p></div>`);
    els.answer.focus();
    return;
  }
...
async function startPractice() {
  try {
    ...
    const response = await fetch("/a11a-radical-expressions.json");
    if (!response.ok) throw new Error(`A.11A practice bank failed to load (${response.status}).`);
    ...
    els.meta.textContent = "Algebra 1 • TEKS A.11A";
    els.skill.textContent = "A.11A • Simplify Numerical Radicals";
    els.sessionDifficulty.textContent = labelDifficulty(difficulty);
    els.sessionCount.textContent = String(count);
    renderQuestion();
  } catch (error) {
    console.error("A.11A practice failed:", error);
