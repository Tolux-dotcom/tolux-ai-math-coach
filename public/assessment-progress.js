(() => {
  ...
  const PENDING_KEY = 'toluxTestPrepPendingProgress:v1';
  ...
  function readPending(storage = safeStorage()) {
    if (!storage) return [];
    try {
  ...
  function writePending(reports, storage = safeStorage()) {
    if (!storage) return;
    try {
      storage.setItem(PENDING_KEY, JSON.stringify(reports));
    } catch {}
  }

  function queuePending(report, storage = safeStorage()) {
    const pending = readPending(storage).filter(
      item => item?.completion_id !== report.completion_id
    );
    pending.push(report);
    writePending(pending.slice(-20), storage);
  }

  function removePending(completionId, storage = safeStorage()) {
    writePending(
      readPending(storage).filter(item => item?.completion_id !== completionId),
      storage
    );
  }
  ...
  async function save(input, dependencies = {}) {
    ...
    try {
      ...
      removePending(report.completion_id, storage);
