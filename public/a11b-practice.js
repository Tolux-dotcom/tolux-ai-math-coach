...
function buildPracticeItems(module, difficulty, count) {
  ...
  if (bank.length < count) throw new Error("A.11B does not yet contain enough verified practice items.");
  const ordered = difficulty === "mixed"
    ? shuffle(bank)
    : [
        ...shuffle(bank.filter(item => normalizeItemDifficulty(item) === difficulty)),
        ...shuffle(bank.filter(item => normalizeItemDifficulty(item) !== difficulty))
      ];
  ...
  return ordered.slice(0, count).map((item, index) => ({
    ...
    hint: formatMathNotation(item.hint_steps?.[0] || item.tutor_behavior || "Identify which exponent law matches the structure first."),
    solution_steps: (item.solution_steps?.length ? item.solution_steps : item.alternate_solution_steps || []).map(step => ({
...
function fullHelpMarkup(item) {
  const steps = item.alternate_solution_steps?.length
    ? item.alternate_solution_steps.map(step => ({ equation: formatMathNotation(step.equation), explanation: formatMathNotation(step.explanation) }))
    : item.solution_steps;
  return `
    <div class="lesson-state lesson-state-success">
      <strong>Another way to think about it</strong>
      <p>First identify the structure: same-base product, same-base quotient, power of a power, zero/negative exponent, or rational exponent. Then apply only that matching exponent law.</p>
    </div>
    ${solutionMarkup({ ...item, solution_steps: steps }, "Step-by-step exponent reasoning")}
...
async function ensureAccess(item) {
  ...
  try {
    ...
    console.error("A.11B practice access failed:", error);
...
async function heartbeat() {
  ...
  try {
    ...
    console.error("A.11B heartbeat failed:", error);
  }
}
...
function renderQuestion() {
  ...
  els.progressLabel.textContent = `A.11B • Question ${number} of ${practiceSession.count}`;
  ...
  els.answer.value = "";
  els.answer.placeholder = "Example: x^7, 1/x^3, or 8";
...
async function checkAnswer() {
  ...
  if (record.attempt_count === 1) {
    ...
    setFeedback(`<div class="lesson-state lesson-state-warning"><strong>Not correct yet</strong><p>${escapeHtml(item.tutor_behavior || "Identify the exponent structure, then apply the matching law.")}</p><button id="a11bShowSolution" class="lesson-primary-button" type="button">Show Answer & Full Solution</button></div>`);
    document.querySelector("#a11bShowSolution")?.addEventListener("click", () => {
      setControlsDisabled(true);
      els.next.hidden = false;
      setFeedback(`<div class="lesson-state lesson-state-success"><strong>Here is the complete solution</strong></div>${solutionMarkup(item)}<p><strong>Final answer:</strong> ${escapeHtml(item.answer_key)}</p>`);
    });
    els.answer.focus();
    return;
  }
...
function completionId() {
  return window.crypto?.randomUUID?.() || `a11b-${Date.now()}-${Math.random().toString(16).slice(2)}`;
}
...
async function saveCompletion(summary) {
  if (!completionReport) {
    completionReport = {
      ...
      module_id: "practice-alg1-a11b-laws-of-exponents",
...
async function finishSession() {
  ...
  try {
    ...
    console.error("A.11B practice completion sync failed:", error);
    status.textContent = "Saved on this device. Tolux will retry account sync from the dashboard.";
  }
}

function advanceQuestion() {
  if (currentIndex >= practiceSession.count - 1) {
    finishSession();
    return;
  }
  currentIndex += 1;
...
function showFatalError(message) {
  ...
  els.questionView.innerHTML = `<div class="lesson-state lesson-state-error"><h2>We could not start this practice session</h2><p>${escapeHtml(message)}</p><a class="lesson-link-button" href="/#practiceModePanel">Choose Practice Settings</a></div>`;
}

async function startPractice() {
  try {
    const params = new URLSearchParams(window.location.search);
    const difficulty = String(params.get("difficulty") || "grade-level").toLowerCase();
    ...
    const response = await fetch("/a11b-laws-of-exponents.json");
    if (!response.ok) throw new Error(`A.11B practice bank failed to load (${response.status}).`);
    const module = await response.json();
    const items = buildPracticeItems(module, difficulty, count);
    practiceSession = { skill: "A.11B", title: "Laws of Exponents", module_id: module.module_id, difficulty, count, items };
    els.title.textContent = practiceSession.title;
    els.meta.textContent = "Algebra 1 • TEKS A.11B";
    els.skill.textContent = "A.11B • Laws of Exponents";
    els.sessionDifficulty.textContent = labelDifficulty(difficulty);
    els.sessionCount.textContent = String(count);
    renderQuestion();
  } catch (error) {
    console.error("A.11B practice failed to start:", error);
...
els.next.addEventListener("click", advanceQuestion);
...
els.stuck.addEventListener("click", async () => {
  ...
  const record = getRecord(item);
  record.hint_count += 1;
  const hints = item.hint_steps || [];
  const hint = hints[Math.min(record.hint_count - 1, Math.max(0, hints.length - 1))] || item.hint || item.tutor_behavior;
  setFeedback(`<div class="lesson-state lesson-state-warning"><strong>Hint ${record.hint_count}</strong><p>${escapeHtml(formatMathNotation(hint || "Identify the exponent law first."))}</p>${record.hint_count >= 3 ? `<p><strong>Answer:</strong> ${escapeHtml(item.answer_key)}</p>${solutionMarkup(item)}` : ""}</div>`);
});
