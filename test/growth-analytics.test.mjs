...
test('rejects unsupported browser events', () => {
  assert.throws(
    () => normalizeGrowthEvent({ eventName: 'subscription_activated' }),
    /Unsupported growth event/
  );
});

test('accepts Stripe lifecycle events only when server marks the source Stripe', () => {
  const event = normalizeGrowthEvent(
    { eventName: 'subscription_activated', plan: 'family' },
    { source: 'stripe' }
  );
...
test('rejects unknown plans', () => {
  assert.throws(
    () => normalizeGrowthEvent({ eventName: 'checkout_started', plan: 'enterprise' }),
    /Unsupported Tolux plan/
  );
});
