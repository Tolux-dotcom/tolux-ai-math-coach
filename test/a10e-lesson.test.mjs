...
test("A.10E resolves as a structured lesson", () => {
  const entry = findCourseModule(catalog, "alg1-a10e-factor-trinomials");
  assert.ok(entry.available_modes.includes("lesson"));
  assert.ok(entry.available_modes.includes("practice"));
  assert.equal(entry.lesson_path, "/a10e-factor-trinomials.json");
});

test("A.10E validates with 30 unique items", () => {
...
test("A.10E covers the complete factoring gate", () => {
  for (const tag of [
    "factor_pairs",
    "gcf_first",
    "perfect_square",
...
test("A.10E accepts standard and typed exponent forms", () => {
  for (const [id, answer] of [
    ...
    assert.equal(
      answersEquivalent(answer, module.items.find(item => item.id === id)),
      true,
      id
    );
  }
...
test("A.10E answer keys are algebraically correct", () => {
  const expected = {
    "A10E-L01": "(x-4)(x+3)",
    "A10E-L02": "(3x+1)(2x+3)",
    "A10E-L03": "(2x-3)²",
    "A10E-M02": "(3x-2)(2x+1)",
    "A10E-X07": "(x+1-√2)(x+1+√2)",
    "A10E-X11": "3(2x-3)(2x+3)"
  };
...
test("A.10E student-facing powers use superscripts", () => {
  for (const item of module.items) {
    assert.doesNotMatch(item.prompt, /\^[2-9]/, item.id);
    assert.doesNotMatch(item.answer_key, /\^[2-9]/, item.id);
  }
});
